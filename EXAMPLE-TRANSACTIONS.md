...
[bitfeed.live](https://bitfeed.live) is the canonical reference: it's the public block explorer run by the `bitfeed-project/bitfeed` team (by mononaut, MIT), and every Bitmap renderer in the ecosystem ports from that codebase. Same caveats as mempool.space's block view apply — bitfeed sizes tiles by tx weight and shows block fullness, so it's not pixel-identical to a Bitmap render, but the packing geometry is the common ancestor.
