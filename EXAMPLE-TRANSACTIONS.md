...
[bitfeed.live](https://bitfeed.live) is the canonical reference: the mempool and block explorer [Bitfeed](https://github.com/bitfeed-project/bitfeed), an MIT-licensed project by mononaut that every Bitmap renderer in the ecosystem ports from. Same caveats as mempool.space's block view apply — Bitfeed sizes tiles by tx weight and shows block fullness, so it's not pixel-identical to a Bitmap render, but the packing geometry is the common ancestor.
