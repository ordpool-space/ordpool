...
- **Accent colour:** Bitcoin orange `$bitcoin: #FF9900` only, aliased `--primary`/`--info`/`--orange`/`--tertiary`; use `var(--primary)`. For UI states use semantic vars `var(--success)`/`var(--warning)`/`var(--danger)`, never invented tints.
...
- **Cube iconography (perspective + lighting are non-negotiable):**
  ...
  - Allowed exception: the block-timeline cube (`.bitcoin-block` in blockchain-blocks / mempool-blocks / stale-list), a 3-face Necker variant with a flat FRONT face carrying content (fee rate, MB, cat21 avatar, tx count).
