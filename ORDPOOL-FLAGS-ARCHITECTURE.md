...
bits in after upstream's static-flag block:
...
`getTransactionFlags`, which OR's in OTS) before the wire write. The
early-return doesn't drop OTS — the OTS pre-enrichment runs above it —
so every re-classification preserves the bit.
...
recomputable. In ordpool, the strip surfaces cannot deliver
`ordpool_ots` via the recompute path: the client has no way to know
which Merkle roots in `OP_RETURN` outputs come from OTS calendars.

The mitigation is `isOtsCommit` — a tristate `boolean | null` field
attached server-side on the two strip surfaces (REST `/api/v1/tx/:txId`
and WS track-tx), consumed client-side by `OtsKnowledgeService`. Full
details in §4.
...
Optional field on `TransactionExtended` (backend) and `Transaction`
(frontend):

```ts
// backend/src/mempool.interfaces.ts:142
// frontend/src/app/interfaces/electrs.interface.ts:35
isOtsCommit?: boolean | null;
```

Tristate semantics:

- `true` — server confirmed this tx is in `ordpoolOtsTxidSet` (OTS commit).
- `false` — server confirmed this tx is **not** in `ordpoolOtsTxidSet`
  (the common case).
- `null` / absent — server didn't compute it; client must derive the
  answer itself (OP_RETURN fast path → lazy probe).
...
- Bulk surfaces that already ship `tx.flags`. Pointless duplication —
  the OTS bit is in `tx.flags` directly there.
...
Implemented in `OtsKnowledgeService.isOtsCommit(tx)` at
`frontend/src/app/services/ordinals/ots-knowledge.service.ts:hasOpReturn`.

Live decision flow in the service:

```ts
async isOtsCommit(tx: Transaction): Promise<boolean> {
  // (1) Server attached the answer.
  if (tx.isOtsCommit === true)  return true;
  if (tx.isOtsCommit === false) return false;

  // (2) OP_RETURN fast path: no OP_RETURN -> impossible to be OTS.
...
The service is integrated into `transaction.utils.ts::getTransactionFlags`
as an optional `otsKnowledge?: OtsKnowledgeService` parameter; the three
component call sites (`transaction.component`, `transaction-raw.component`,
`tracker.component`) inject and pass it through. The OTS bit is OR'd
into the returned `flags` bigint after parser-derived bits.

### 4.3 Lazy backend endpoint

Live as `GET /api/v1/ordpool/ots/is-commit/:txid` in
`ordpool.routes.ts:$isOtsCommit`:

```
GET /api/v1/ordpool/ots/is-commit/<64-hex-txid>  ->  { result: boolean }
```
...
There is no batch variant: the OP_RETURN fast path eliminates ~99% of
would-be lookups, so a single-tx GET is the right shape. If usage
patterns shift, a `POST /api/v1/ordpool/ots/is-commit { txids: string[] }`
can be added later — additive, no breakage.

### 4.4 Client-side cache

In-memory `Map<txid, { value: boolean; expiry: number | null }>`
inside `OtsKnowledgeService` (not `sessionStorage` — the cache is
ephemeral, regenerated on page reload, and the cost is negligible
because of the OP_RETURN fast path). One subtle detail:

- `true` results are **monotonic** — once a tx is known to be an OTS
  commit, that's permanent. `expiry: null` → cache forever.
- `false` results are **NOT monotonic** — the poller might learn about
  the tx after the negative response. `expiry: Date.now() + 60_000` →
  TTL matches the route's `Cache-Control`.
- **Probe failures degrade to `false` WITHOUT caching the failure** —
  so a transient backend hiccup doesn't poison the cache for the
  client's session. Retries are free.

Pinned by 11 tests in `ots-knowledge.service.spec.ts` covering every
branch of the three-source logic + cache semantics + graceful
degradation under probe failure.
...
- **OTS pre-enrichment** (runs above the early-return): a tx that was
  OTS-eligible before restart but lacked the bit in the snapshot gets
  the bit applied here, provided `ordpoolOtsTxidSet.bootstrap()` has
  completed (see §6.1).
...
- **Behaviour**: full static-flag derivation + parser-derived bits +
  ...
  batch, the OTS bit is set on first classification.
...
- **Early-return**: fires.
- **OTS pre-enrichment** (runs above the early-return): if the poller
  observed the calendar batch between mempool ingest and block
  confirmation, the OTS bit is added here. This is the load-bearing
  call site for the eventual-consistency contract — without it, the
  block summary would inherit the stale mempool classification.
...
- **`tx.flags` on entry to `getTransactionFlags`**: `undefined` (just
  cleared).
- **Early-return**: does not fire.
- **Behaviour**: full recomputation including OTS pre-enrichment.
...
This is the only category where `tx.flags` cannot carry the OTS bit —
the indexer-derived classification is not part of the recompute. The
server therefore attaches `tx.isOtsCommit` instead (see §4), and the
frontend's `OtsKnowledgeService` applies the bit on the client.
...
**Mitigation:** OTS pre-enrichment runs unconditionally above
`getTransactionFlags`'s early-return. Every re-classification re-checks
the set, so any subsequent call picks up the bit once the poller
catches up.
...
All of these refresh the bit through the call sites enumerated in §5.

Pinned by `backend/src/api/ordpool-flags-ots-retroactive.test.ts` (10
tests; mutation-test: 4 fail if the OTS pre-enrichment block is moved
back below the early-return).

### 6.3 Strip wire surfaces: OTS bit doesn't reach the frontend via `tx.flags`

Single-tx REST GET and WS track-tx ship `tx.flags = undefined`. The
frontend can recompute parser-derived bits from witness bytes but
cannot recompute OTS.

**Mitigation:** the `isOtsCommit` tristate field — see §4. Server
attaches it on both strip surfaces; frontend's `OtsKnowledgeService`
consumes it with an OP_RETURN fast path and a lazy backend probe for
the remaining ambiguous cases.

### 6.4 BigInt arithmetic gotcha

Every ordpool flag lives at bit ≥ 48; the highest (`ordpool_ots`) is at
bit 81. JavaScript's bitwise operators (`|`, `&`, `^`, `<<`, `>>`)
coerce to int32 before computing — every ordpool bit gets truncated to
zero.

**Mitigation:** all code that touches ordpool flags converts to
`BigInt` first, ORs in `BigInt` space, then converts back to `Number`
